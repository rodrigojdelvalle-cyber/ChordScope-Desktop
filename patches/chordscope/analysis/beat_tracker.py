...
def _estimate_meter(beats: np.ndarray, downbeats: np.ndarray) -> int:
    ...
    for a, b in zip(downbeats[:-1], downbeats[1:]):
        n = int(np.sum((beats >= a - 0.08) & (beats < b - 0.08)))
        if 2 <= n <= 12:
            counts.append(n)
    if not counts:
        return 4
...
def _align_grid(beats: np.ndarray, downbeats: np.ndarray, duration: float, meter: int) -> np.ndarray:
    ...
    if not np.isfinite(period) or period <= 0:
        return beats
    out = beats.tolist()
    while out and out[0] - period >= -0.12:
        out.insert(0, out[0] - period)
    while out and out[-1] + period <= duration + 0.12:
        ...
        if indices:
            ...
            if phase:
                ...
                for k in range(phase, 0, -1):
                    t = first - k * period
                    if t >= -0.08:
                        prepend.append(max(0.0, t))
                grid = np.asarray(prepend + grid.tolist()) if len(prepend) == phase else grid[phase:]
    dedup=[]
    for x in grid:
        if not dedup or x-dedup[-1] > period*0.22:
            dedup.append(float(x))
    return np.asarray(dedup, dtype=float)


def detect_beats(audio_path: str | Path, duration: float, progress: Progress | None = None) -> dict:
    device = _device()
    try:
        if progress:
            progress(5, f"Beat This! · {device.upper()}")
        ...
        if len(beats) < 2:
            raise RuntimeError("Beat This devolvió menos de dos beats")
        period=float(np.median(np.diff(beats))); bpm=60.0/period
        meter=_estimate_meter(beats,downbeats)
        grid=_align_grid(beats,downbeats,duration,meter)
        if progress:
            progress(100,f"Beat This! · {bpm:.1f} BPM · {meter}/4")
        return {"beats":grid.tolist(),"downbeats":downbeats.tolist(),"bpm":round(bpm,2),"period":period,"meter":meter,"confidence":0.92,"engine":"Beat This! final0"}
    except Exception as exc:
        if progress:
            progress(35, f"Beat This! fallback · DSP nativo ({type(exc).__name__})")
        y,sr=load_mono(audio_path,sr=22050)
        bpm,beats,conf=estimate_tempo_native(y,sr)
        grid=_align_grid(np.asarray(beats),np.asarray([]),duration,4)
        period=60.0/max(1e-6,bpm)
        if progress:
            progress(100,f"DSP tempo · {bpm:.1f} BPM")
